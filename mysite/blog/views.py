...
#Blog main page view with pagination
class PaginatedMainView(SingleObjectMixin, ListView):
	paginate_by = 4
	template_name = "blog/blog_main.html"

	def get(self, request, *args, **kwargs):
		self.object = self.get_object(queryset=Article.objects.all())
		return super(PaginatedMainView, self).get(request, *args, **kwargs)

	def get_context_data(self, **kwargs):
		context = super(PaginatedMainView, self).get_context_data(**kwargs)
		#context['latest_article_list']
		context['category_list'] = Category.objects.order_by('order')
		context['article_list'] = Article.objects.order_by('-written_at')
		context['user'] = self.request.user
		return context
